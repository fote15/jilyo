<template>
  <div>
    <vue-final-modal v-model="$store.state.ShowModal" @closed="closed">
      <div class="cont">
        <div class="form sign-in">
          <h2 class="welcome__text__login">Вход</h2>
          <label>
            <span>Ваш номер</span>
            <the-mask
              v-model="rowVal"
          ...
          <label>
            <span>Пароль</span>
            <input @keypress="isNumber($event)" type="password" v-model="password" />
            <span class="text-danger">{{errorText}}</span> 
          </label>
          <button type="button" class="submit" @click="Register">Войти</button>
          <button type="button" class="fb-btn">
            <span @click="$store.state.ShowModal=!$store.state.ShowModal;$router.push('signup')">Зарегистрироваться</span>
          </button>
        </div>
      </div>
    </vue-final-modal>
...
export default {
  ...
  methods: {
    ...
    showModal1() {
      this.$store.state.showModal = !this.showModal;
    },
    closed() {
