<template>
	<div>
		<div class="posts">
			<div class="postsTitle">Истекшие</div>
			<div class="line"></div>
			<table class="postsTable">
				...
				<tr v-for="x in postsData">
					...
					<td>{{x.status==1 ? 'На рассмотрении':"" }}{{x.status==2 ? 'Опубликован':"" }}{{x.status==3 ? 'Скрыт':"" }}{{x.status==4 ? 'Истекшие':"" }}</td>
...
export default {
	...
	mounted() {
		...
		axios.post('https://xn--e1abgm9f.kz/api/api/post-list-my-expire',{}, { headers: { 'Authorization': `Token ${this.$store.state.token}` }})
			.then((e) => {
